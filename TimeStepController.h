...
namespace PBD
{
	...
	protected:
		...
		*/
		...
		void positionConstraintProjection2(SimulationModel& model);
		void velocityConstraintProjection(SimulationModel& model);

		static void contactCallbackFunction(unsigned int first, unsigned int second, const unsigned int contactType,
			const unsigned int bodyIndex1, const unsigned int bodyIndex2, const unsigned int bodyIndex3, const unsigned int bodyIndex4,
	...
	public:
		...
		void step(SimulationModel& model, Configuration& conf);
		void reset();
